...
-- 声明两个游标，第二个游标使用到第一个的查询结果
    DECLARE order_csr CURSOR FOR SELECT
                                   ID,
                                   ACCOUNT_OPER.OPER_TYPE,

                                 FROM ACCOUNT_OPER
                                 WHERE PAY_TYPE <> 'ORDER_PAY';
...
-- 第一个循环
    order_loop: LOOP -- Loop through org_grade
      ...
      THEN
        INSERT INTO MONEY_SAVE (ID, PAY_TYPE, REMIT_FEE, USE_CHECK, CHECK_NUMBER, NUMBER, PROXY, MONEY)
        VALUES (_AID, )
      END IF;
