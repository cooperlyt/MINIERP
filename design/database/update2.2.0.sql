...
SELECT
  *
FROM ACCOUNT_OPER
WHERE OPER_TYPE <> 'ORDER_PAY' AND OPER_TYPE <> 'ORDER_SAVINGS'
      AND OPER_TYPE <> 'PRE_DEPOSIT' AND OPER_TYPE <> 'PRE_DEPOSIT_BY_ORDER'
      AND OPER_TYPE <> 'ORDER_CANCEL_SAVINGS' AND OPER_TYPE <> 'ORDER_BACK';
  ...
  BEGIN
    ...
    order_loop: LOOP -- Loop through org_grade
      ...
      THEN
        ...
        IF _ORDER_PAY_TYPE = 'EXPRESS_PROXY'
        THEN
          UPDATE MINI_ERP.CUSTOMER SET PROXY_ACCOUNT_MONEY = PROXY_ACCOUNT_MONEY + _RRMONEY WHERE ID = _CUSTOMER_ID;
        END IF;

        IF _ORDER_PAY_TYPE <> 'EXPRESS_PROXY'
        THEN
          UPDATE MINI_ERP.CUSTOMER SET ACCOUNT_MONEY = ACCOUNT_MONEY + _RRMONEY WHERE ID = _CUSTOMER_ID;
        END IF;
...
SELECT
  *
FROM ACCOUNT_OPER
WHERE OPER_TYPE = 'PRE_DEPOSIT_BY_ORDER';


DELETE
FROM ACCOUNT_OPER
WHERE CUSTOMER_ORDER IN (SELECT CUSTOMER_ORDER.ID  FROM CUSTOMER_ORDER
